...
#define TOPIC       "cr_Jordan/feeds/smart-home.momentaryled"
...
int msgarrvd(void *context, char *topicName, int topicLen, MQTTClient_message *message) {
    ...
    string toggleStatus(payloadptr);

    MYGPIO toggleLED(2);
    toggleLED.setDirection(OUTPUT);
    if (toggleStatus == "1") {
      toggleLED.setValue(HIGH);
    } else if (toggleStatus == "0") {
      toggleLED.setValue(LOW);
    }
